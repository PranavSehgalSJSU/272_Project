...
export default function Page() {
  ...
  const [error, setError] = useState("");
  const [success, setSuccess] = useState("");
  const [sendToAll, setSendToAll] = useState(false); // State for the "Send to all users" checkbox
  ...
  const canSubmit = form.sender && (form.receiver || sendToAll) && form.message;

  function buildJSON() {
    return {
      sender: form.sender,
      receiver: sendToAll ? "*" : form.receiver.trim(), // Use "*" if the checkbox is checked
      receivers: sendToAll ? ["*"] : form.receiver.split(","), // Split receivers if not "send to all"
      message: form.message,
      header: form.header,
      mode: form.mode === "both" ? "*" : form.mode,
      token: localStorage.getItem("token") || "",
        ...
        <form onSubmit={onSubmit} className="mt-6 space-y-5">
          ...
          <div>
            ...
            <input
              ...
              required={!sendToAll} // Make receiver field required only if "Send to all users" is not checked
            />
          </div>

          {/* Add the checkbox for "Send to all users" */}
          <div className="mt-3">
